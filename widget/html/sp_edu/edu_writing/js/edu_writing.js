    		...
    		if(ret.status==1){
    			if(ret.data&&ret.data.length>0){
	    			for(var i=0;i<ret.data.length;i++){
		    			$('#list').append(
		    				'<li class="mb-12em over-h" onclick="openInfo(\'edu_writing_info_header\','+ret.data[i].article_id+')">'+
	...
	//			                        '<div class="pull-right color-a4 f-14em pt-5em">2188浏览</div>'+
	    		...
	    		api.hideProgress();//隐藏进度提示框
    			 ...
    			 if(page==1){
    			 	$("#list #more").remove();
					$("#list").append(
						'<div class="aui-user-view-cell aui-text-center clearfix color-99" id="more">暂无考试内容</div>'
					);	
	    			api.hideProgress();//隐藏进度提示框
    			 }else if(page>1){
    			 	$("#list #more").remove();
