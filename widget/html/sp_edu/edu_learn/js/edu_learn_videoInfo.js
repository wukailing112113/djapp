apiready=function(){
	getEduMaterialInfo();//调用（获取视频资料详情）接口
	//接收监听
	...
	api.ajax({
	    ...
	    data: {//传输参数
	    	values: { 
	    		...
	    		"material_id":api.pageParam.type,//视频id
	        }
	    }
    },function(ret,err){
    	if(ret){
    		if(ret.status==1){
              ...
              material_attach = ret.data.material_attach.split('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;');
    			...
    			$('#material_vedio').attr('src',localStorage.url+ret.data.material_vedio);//视频路径
	...
	api.ajax({
    	...
    	if(ret){
    		...
    		if(ret.data.is_has==1){
    			alert("您今天已经看过此视频，请浏览其他视频获得积分!");
    		}else{
    			alert('获得'+ret.data.integral_video+'积分');
    			//发送监听
			     api.sendEvent({
				 name: 'myEvent_xw2',
				 extra: {
    			...
    			//api.toast({msg: '获得'+ret.data.integral_video+'积分' ,duration:3000,location: 'top'});
    		}
...
//遮罩层--------------------------------------------------------
$("#edu_video_input").focus(function(){
 $(".edu_video_mask").removeClass("di-n");
 $(this).hide();
});
//关闭遮罩层
$(".mask_close").click(function(){
	//exec();
	 $(".edu_video_mask").addClass("di-n");
	 $("#edu_video_input").show();
});
//打开学习笔记页面-------------------------------------------------------------
