...
apiready = function () {
    get_workplan_list('ready_in','11');//未开展
    get_workplan_list('in_turn','21,52');//进行中
    get_workplan_list('wait_approval','31');//待审核
    get_workplan_list('done','31,32');//已完成
    get_workplan_list('break','51');//已中止
       
}
//处理加载数据方法
function get_workplan_list(id,status){ 
    //showProgress();//加载等待   
	//ajax获取数据处理
    	...
    	if(ret){
    		if(ret.status == 1){
				...
				for(var i = 0; i < 10; i++){
				    if(list[i]!=null){
				        $("#"+id+"_list").append(
                            '<li class="" onclick="openInfoWin(\'work_infoHeader\','+list[i].workplan_id+',\'work\')">'+
                                ...
                                '<div class="aui-timeline-item">'+                                 
                                    ...
                                    '<div class="aui-timeline-body p-li aui-border-b mb-5 mt-8">'+
                                        ...
                                        '<p class="di-i mb-5">'+
                                            ...
                                            '</span>'+
                                            '<span class="fsize-12 fcolor-gray1 aui-pull-right lh-22">指派人:<em>'+list[i].staff_name+'</em></span>'+
                                        '</p>'+
                                    '</div>'+                                   
                                '</div>'+
                            '</li>'
				...
				if(list.length!=0){
                   $("#"+id+"_length").removeClass("di-none");//列表长度
                   $("#"+id+"_length").html(list.length);//列表长度                   
                }
				//有十条以上数据的时候显示加载更多
                    if(list.length>9){
                        $("#"+id+"_list").append(
                            '<div id="more'+id+'" class="aui-list-view-cell pr-0 border-b-none">'+
                                '<span class="aui-pull-right w-30b text-r lh-14 aui-text-info" onclick="openNewWin(\'work_list_secondHeader\',\''+status+'\',\'work\')">查看更多>></span>'+
                            '</div>'
                        )                      
                    }	
...
//打开新页面
function openInfoWin(winName,workplan_id,type){
	api.openWin({
	    name: winName,
...
function openNewWin(winName,status,type){
    api.openWin({
        name: winName,
        url: 'header/'+winName+'.html',
        pageParam:{
