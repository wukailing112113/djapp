...
function getHrCommunistList(departmentIdList){
	var url = localStorage.url+"/index.php/Api/public/staff_list";
	api.ajax({
	    url:url,
	    method: 'post',
	    timeout: 100,
    	...
    	if(ret){
    		if(ret.status==0){
    		//alert(ret.data[0].staff_name)
    			if(api.pageParam.workplan_type!='workplan_type'){
    			
	    			if(ret.data&&ret.data.length>0){
		    			for(var i=0;i<ret.data.length;i++){
		    				$('#personnelBox').append(
			    				'<li confirm="flase" class="over-h bb-1-df pt-10em" communistNo="'+ret.data[i].staff_no+'" onclick="ChoosePeopleFun(this)">'+
									'<img class="pull-left w-40em h-40em bor-ra-100b mr-12em mb-10em" src="'+((ret.data[i].staff_avatar)?(localStorage.url+ret.data[i].staff_avatar):("../../../statics/images/images_oa/oa_img_head.png"))+'" alt="" />'+
									'<div class="pull-left w-270em">'+
										'<p class="f-16em lh-16em color-33 f-w pt-5em pb-8em name">'+ret.data[i].staff_name+'</p>'+
										'<p class="f-16em lh-16em color-75 pb-12em">'+InterceptField(ret.data[i].staff_dept_name,'所属支部未找到',16)+'</p>'+
									'</div>'+
		    		...
		    		if(ret.data&&ret.data.length>0){
		    			for(var i=0;i<ret.data.length;i++){
		    				$('#personnelBox').append(
			    				'<li confirm="flase" class="over-h bb-1-df pt-10em" communistNo="'+ret.data[i].staff_no+'" onclick="ChoosePeopleRadio(this)">'+
									'<img class="pull-left w-40em h-40em bor-ra-100b mr-12em mb-10em" src="'+((ret.data[i].staff_avatar)?(localStorage.url+ret.data[i].staff_avatar):("../../../statics/images/images_oa/oa_img_head.png"))+'" alt="" />'+
									'<div class="pull-left w-270em">'+
										'<p class="f-16em lh-16em color-33 f-w pt-5em pb-8em name">'+ret.data[i].staff_name+'</p>'+
										'<p class="f-16em lh-16em color-75 pb-12em">'+InterceptField(ret.data[i].staff_dept_name,'所属支部未找到',16)+'</p>'+
